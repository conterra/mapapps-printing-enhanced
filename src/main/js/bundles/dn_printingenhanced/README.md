...
## Usage
1. First, add the bundle `dn_printingenhanced` to your app.
2. Then configure it.

The following tool can be added to a toolset to show and hide the printing enhanced dialog window:
...
| `layoutTemplatesInfoTaskName` | String             |                                    | ```Get Layout Templates Info Task``` | This is the name of the REST endpoint of the ArcGIS Server _GetLayoutTemplatesInfo_ task. For example, if the URL to the ArcGIS Server _GetLayoutTemplatesInfo_ endpoint is https://www.example.com/server/rest/services/support/PrintMap/GPServer/Get%20Layout%20Templates%20Info, then the value of the `layoutTemplatesInfoTaskName` property is `Get%20Layout%20Templates%20Info`, where the `%20` in the value are URL-encoded space characters. |
...
### Using custom text elements in the print template

In an ArcGIS Server print template, you can define variables for text content that should appear in the print, so-called "custom text elements".
Printing enhanced will display input fields for these variables, so the user can specify custom values, e.g., a custom title for the printed map.
To create a print template with custom text elements in ArcGIS Server, follow the guide [Share custom layouts for printing from ArcGIS Pro](https://enterprise.arcgis.com/en/server/latest/publish-services/windows/tutorial-publish-additional-layouts-for-printing-with-arcgis-pro.htm).

When the template is available, configure the properties `printing.Config.url` as described above, and `printing.Config.layoutTemplatesInfoTaskName` as described in the table above.
