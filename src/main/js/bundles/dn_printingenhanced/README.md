...
| visibleUiElements           | Object             |                                |                                  | Controls visibility of UI elements. `legendModes` is an array listing which legend mode radio options are offered, in display order, chosen among `integratedLegend`, `legendOwnPage` and `noLegend` (e.g. `["legendOwnPage", "noLegend"]` to leave out `integratedLegend`). With fewer than 2 entries no UI is shown and that mode (or `noLegend` if the array is empty) is used automatically. The legend mode choice is shared between the layout and map-only tabs; since a map-only export is a plain image with no way to embed a legend, `integratedLegend` isn't offered there and is treated as `legendOwnPage` (or `noLegend` if that isn't configured either). |
